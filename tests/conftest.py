...
# ocr_worker defaults OCR_CACHE_DIR to /app/.paddle_cache, which only exists
# inside the Docker image. Outside of it (e.g. CI runners, local dev) that
# path isn't writable, so point it at a temp dir before ocr_worker imports.
os.environ.setdefault("OCR_CACHE_DIR", str(Path(tempfile.gettempdir()) / "paddle_ocr_test_cache"))
